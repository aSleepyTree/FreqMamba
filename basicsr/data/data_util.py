...
def paired_paths_from_meta_info_file(folders, keys, meta_info_file,
    ...
    return paths
def paired_paths_from_folder_test1200(folders, keys, filename_tmpl):
    ...
    # print(input_paths,input_folder)
    rain_1 = os.path.join(os.path.join(input_folder,input_paths[0]),'train2018new')
    rain_2 = os.path.join(os.path.join(input_folder,input_paths[1]),'train2018new')
    rain_3 = os.path.join(os.path.join(input_folder,input_paths[2]),'train2018new')
    # print(rain_1)
    gt_paths = list(scandir(rain_1))
    # gt_paths2 = list(scandir(rain_2))
    # gt_paths3 = list(scandir(rain_3))
    # gt_paths = gt_paths1 + gt_paths2 + gt_paths3
    # print(len(gt_paths))
    ...
    for idx in range(len(gt_paths)):
        path1 = os.path.join(rain_1,gt_paths[idx])
        path2 = os.path.join(rain_2,gt_paths[idx])
        path3 = os.path.join(rain_3,gt_paths[idx])
        paths.append(
            dict([(f'{input_key}_path', path1),
                  (f'{gt_key}_path', path1)]))
        paths.append(
            dict([(f'{input_key}_path', path2),
                  (f'{gt_key}_path', path2)]))
        paths.append(
            dict([(f'{input_key}_path', path3),
                  (f'{gt_key}_path', path3)]))
    return paths
def paired_paths_from_folder_test1200_test(folders, keys, filename_tmpl):
    """Generate paired paths from folders.
    ...
    input_paths = list(os.listdir(input_folder))
    # print(len(input_paths),input_folder)
    ...
    paths = []
    for idx in range(len(gt_paths)):
        path = os.path.join(input_folder,gt_paths[idx])
        paths.append(
            dict([(f'{input_key}_path', path),
                  (f'{gt_key}_path', path)]))
    return paths
