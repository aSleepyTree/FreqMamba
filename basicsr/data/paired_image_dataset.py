...
class Dataset_PairedImage(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        lq_path = self.paths[index]['lq_path'].replace('GT','hazy')
        # print(lq_path)
        # exit()
...
class Dataset_test1200(data.Dataset):
    """Paired image dataset for image restoration.
    ...
    def __init__(self, opt):
        super(Dataset_test1200, self).__init__()
        self.opt = opt
        # file client (io backend)
        self.file_client = None
        self.io_backend_opt = opt['io_backend']
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            self.paths = paired_paths_from_folder_test1200(
                [self.lq_folder, self.gt_folder], ['lq', 'gt'],
                self.filename_tmpl)
    ...
    def __len__(self):
        return len(self.paths)
class Dataset_test1200_test(data.Dataset):
    """Paired image dataset for image restoration.
    ...
    def __init__(self, opt):
        super(Dataset_test1200_test, self).__init__()
        self.opt = opt
        # file client (io backend)
        self.file_client = None
        self.io_backend_opt = opt['io_backend']
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            self.paths = paired_paths_from_folder_test1200_test(
                [self.lq_folder, self.gt_folder], ['lq', 'gt'],
                self.filename_tmpl)
