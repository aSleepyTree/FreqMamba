...
def main():
    ...
    if rank <= 0:  # normal training (rank -1) OR distributed training (rank 0)
        ...
        if opt['use_tb_logger'] and 'debug' not in opt['name']:
            ...
            tb_logger = SummaryWriter(log_dir='./tb_logger/' + util.get_timestamp() +'/'+opt['name'])
    else:
    ...
    #### resume training
    if resume_state:
        # resume_state['epoch']= 4142
        # resume_state['iter'] = 116000
                ...
                if opt['model'] in ['sr', 'srgan'] and rank <= 0:  # image restoration validation
                    ...
                    if opt['dist']:
                        ...
                        if rank == 0:
                            ...
                            print('========-===========:',psnr_total_avg,best_psnr)
                            if psnr_total_avg > best_psnr:
                                best_psnr = psnr_total_avg
                                log_s = '### BestPsnr: {:.4e}||# Validation # PSNR: {:.4e}:'.format(best_psnr, psnr_total_avg)
                            else:
                        ...
                        if psnr_total_avg > best_psnr:
                            best_psnr = psnr_total_avg
                            model.save('best')
                            log_s = '### BestPsnr: {:.4e}||# Validation # PSNR: {:.4e}:||Validation # SSIM: {:.4e}:'.format(best_psnr, psnr_total_avg, ssim_total_avg)
                        else:
                            log_s = '# Validation # PSNR: {:.4e}:'.format(psnr_total_avg)
                        for k, v in psnr_rlt_avg.items():
                            log_s += ' {}: {:.4e}'.format(k, v)
